...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
    ...
    for (x, y, w, h) in features:
        ...
        if id == 1:
            cv2.putText(img, "Deepika Padukone", (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2,
                        cv2.LINE_AA)
            arduinoSerialData.write('H'.encode())
            time.sleep(5)
            arduinoSerialData.write('N'.encode())
            time.sleep(2)
        elif id == 2:
            cv2.putText(img, "NarendraModi", (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
            arduinoSerialData.write('R'.encode())
            time.sleep(5)
            arduinoSerialData.write('N'.encode())
            time.sleep(2)
        elif id == 3:
            cv2.putText(img, "Shivani", (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
            arduinoSerialData.write('H'.encode())
            time.sleep(5)
            arduinoSerialData.write('N'.encode())
            time.sleep(2)
        elif id == 4:
            cv2.putText(img, "other empoyee", (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
            time.sleep(2)
            arduinoSerialData.write('N'.encode())
            time.sleep(2)
        else:
